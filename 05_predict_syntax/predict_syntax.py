...
def predict_udpipe(experiment_layer: str, model: str, collection_name: str, i: int) -> None:
    parse_test = 'udpipe --parse ../03_train_models/output/%s/%s/%s/model_%s.output ' \
                 '../04_train_models/output/%s/%s/%s/test_%s.conllu > ' \
                 '../04_train_models/output/%s/%s/%s/parsed_test_%s.conllu 	 ' % (
                     collection_name, experiment_layer, model, i, collection_name, experiment_layer, model, i,
                     collection_name, experiment_layer,
                     model, i)
    parse_train = 'udpipe --parse ../04_train_models/output/%s/%s/%s/model_%s.output ' \
                  '../04_train_models/output/%s/%s/%s/train_%s.conllu > ' \
                  '../04_train_models/output/%s/%s/%s/parsed_train_%s.conllu ' % (
                      collection_name, experiment_layer, model, i, collection_name, experiment_layer, model, i,
...
def predict_maltparser(experiment_layer: str, model: str, collection_name: str, i: int) -> None:
    options = arguments()
    parse_test = 'java  -jar %s -c model_%s -i ../../../../../03_create_training_testing_data/output/%s/%s/test_%s.conllu ' \
                 '-o ../../../../../04_train_models/output/%s/%s/%s/parsed_test_%s.conllu -m parse ' % (
                     options["path_to_model"], i, collection_name, experiment_layer,
                     i, collection_name,experiment_layer,model, i)
    parse_train = 'java  -jar %s -c model_%s -i ../../../../../03_create_training_testing_data/output/%s/%s/train_%s.conllu ' \
                  '-o ../../../../../04_train_models/output/%s/%s/%s/parsed_train_%s.conllu -m parse' % (
                      options['path_to_model'], i,collection_name,experiment_layer, i,collection_name,experiment_layer,model, i)

    subprocess.call(parse_test, shell=True,
                    cwd='../04_train_models/output/%s/%s/%s/' % (collection_name, experiment_layer, model))
    subprocess.call(parse_train, shell=True,
                    cwd='../04_train_models/output/%s/%s/%s/' % (collection_name, experiment_layer, model))


def arguments():
    path_to_model = '../../../../../04_train_models/resources/maltparser-1.9.2/maltparser-1.9.2.jar'
