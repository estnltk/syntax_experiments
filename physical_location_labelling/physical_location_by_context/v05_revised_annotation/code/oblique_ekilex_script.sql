...
SELECT w.value AS word,
            CASE 
                ...
                WHEN st.code IN (
                		...
                		'taim', 'objekt_loodus', 'objekt', 'osa', 'nähtus_loodus') 
                    THEN 'SOBIB'
...
SELECT w.value AS word,
            CASE 
                WHEN st.code IN (
                    ...
                    THEN 'SEISUND'
                WHEN st.code IN (
                	'nähtus_psühh', 'nähtus', 'nähtus_loodus', 'omadus_psühh', 
                	'abstr_asend/suund', 'abstr_konkr_omadus') 
                    THEN 'SOBIB'
                ELSE 'OTHER'
            END
         AS semantic_types
...
--QUERY 13: LEIA KÕIK SÕNAD, MILLEL ON 1 VÕI MITU ELUSUSEGA SOBIVAT MÄRGENDIT
--võta ainult elusussõnad
WITH words_with_target_types AS (
    SELECT DISTINCT w.value AS word
    FROM word w
    JOIN lexeme l ON l.word_id = w.id 
...
word_type_counts AS (
    SELECT word, 
           COUNT(DISTINCT semantic_types) AS type_count,
           BOOL_OR(semantic_types = 'ELUS') AS has_elus,
           BOOL_OR(semantic_types = 'SOBIB') AS has_sobib,
           BOOL_OR(semantic_types = 'OTHER') AS has_other
    FROM word_semantic_types
    GROUP BY word
)
--vali sellised sõnad, millel on kas 1. üks semantiline tüüp ja see on sündmus või
-- 2. kaks semantilist tüüpi ja need on elus ja sobib
SELECT 
    wtc.word
FROM word_type_counts wtc
WHERE (
