...
class DbReader(BaseReader):
    ...
    def __init__(
        ...
        storage = PostgresStorage(
            pgpass_file=pgpass_file, schema=schema, role=role, temporary=temporary
        )
    ...
    def get_collections(
        self, col_ids=None, shuffle=False, queries=None, count=None, progressbar=None
    ):
        if col_ids is None:
            col_ids = []
        if queries is None:
        ...
        my_select = self.__collection.select(
            query=q, progressbar=progressbar, layers=self._layers, return_index=True
        )
