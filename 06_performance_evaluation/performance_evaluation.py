...
def evaluate_model(experiment_layer: str, model: str, collection_name: str, i: int) -> None:
    ...
    """
    if model == 'malt':
        evaluate_test = 'java -jar ../04_train_models/resources/MaltEval-dist/dist-20141005/lib/MaltEval.jar -s ' \
                        '../04_train_models/output/%s/%s/%s/parsed_test_%s.conllu -g ' \
                        '../03_create_training_testing_data/output/%s/%s/test_%s.conllu ' \
                        '--Metric ' \
                        'LAS;UAS;LA > ../04_train_models/output/%s/%s/%s/result_test_%s.txt' % (
                            collection_name, experiment_layer, model, i, collection_name, experiment_layer, i,
                            collection_name, experiment_layer, model, i)
        evaluate_train = 'java -jar ../04_train_models/resources/MaltEval-dist/dist-20141005/lib/MaltEval.jar -s ' \
                         '../04_train_models/output/%s/%s/%s/parsed_train_%s.conllu -g ' \
                         '../03_create_training_testing_data/output/%s/%s/train_%s.conllu ' \
                         '--Metric ' \
                         'LAS;UAS;LA > ../04_train_models/output/%s/%s/%s/result_train_%s.txt' % (
                             collection_name, experiment_layer, model, i, collection_name, experiment_layer, i,
                             collection_name, experiment_layer, model, i)
    elif model == 'udpipe':
        evaluate_test = 'udpipe --accuracy --parse ../03_create_training_testing_data/output/%s/%s/%s/model_%s.output ' \
                        '../04_train_models/output/%s/%s/%s/test_%s.conllu > ' \
                        '../04_train_models/output/%s/%s/%s/result_test_%s.txt' % (
        ...
        evaluate_train = 'udpipe --accuracy --parse ../03_train_models/output/%s/%s/%s/model_%s.output ' \
                         ...
                         '../03_train_models/output/%s/%s/%s/result_train_%s.txt' % (
                             collection_name,experiment_layer,  model, i, collection_name, experiment_layer, model, i,
                             collection_name,experiment_layer, model, i)
    else:
