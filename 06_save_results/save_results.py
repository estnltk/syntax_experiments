...
def save_results(collection: PgCollection, collection_name: str) -> None:
    ...
    for experiment in os.listdir('../03_train_models/output/%s/' % collection_name):
        splits_malt = pd.read_csv('../03_train_models/output/%s/%s/malt/splits.csv' % (collection_name, experiment))
        splits_udpipe = pd.read_csv('../03_train_models/output/%s/%s/udpipe/splits.csv' % (collection_name, experiment))
        ...
        for file in os.listdir('../03_train_models/output/%s/%s/udpipe/' % (collection_name, experiment)):
            ...
            if 'parsed_test' in file:
                layers = add_to_dictionary(layers, '../03_train_models/output/%s/%s/udpipe/%s' % (
                    collection_name, experiment, file),
                                           splits_udpipe, experiment, 'udpipe')
