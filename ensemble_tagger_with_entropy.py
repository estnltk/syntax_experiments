#
#  Tags dependency syntactic analysis with an ensemble of Stanza's models and records predictions' entropy.
...
class StanzaSyntaxEnsembleTaggerWithEntropy(Tagger):
    """
    Tags dependency syntactic analysis with an ensemble of Stanza's models and records predictions' entropy.
    ...
    conf_param = ['add_parent_and_children', 'aggregation_algorithm', 'syntax_dependency_retagger',
                  ...
                  'taggers', 'remove_fields', 'replace_fields', 'find_entropy', 'random_pick_seed', 
                  '_random1', 'random_pick_max_score_seed', '_random2']
    ...
    def __init__(self,
        ...
        if self.find_entropy:
            self.output_attributes += ('entropy', 'votes')
    ...
    def _make_layer(self, text, layers, status):
        ...
        # Find predictions' uncertainty
        words_entropy = None
        if self.find_entropy:
            words_entropy = \
                find_prediction_entropy(parent_layer, parsed_texts, add_model_votes=True)
        ...
        for token, span in zip(extracted_words, parent_layer):
            ...
            if words_entropy is not None:
                w_entropy = words_entropy[global_word_id]
                attributes['entropy'] = w_entropy['entropy']
                attributes['votes'] = w_entropy['votes']
...
def find_prediction_entropy(words_layer, parsed_texts, add_model_votes=True, add_sentence_ids=False):
    '''
    ...
    If add_sentence_ids, then adds sentence id-s to results.
    '''
    word_id = 0
    sentence_id = 0
    sentence_word_id = 0
    ...
    while word_id < len( words_layer ):
        ...
        normalized_voting_table = \
            [voting_table[k]/len(parsed_texts.items()) for k in voting_table.keys()]
        e = entropy(normalized_voting_table)
        r = {'entropy': e}
        if add_model_votes:
            r['votes'] = voting_table.most_common()
        if add_sentence_ids:
